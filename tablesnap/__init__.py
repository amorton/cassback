...
# work. Sub classes from :cls:`tablesnap.SubCommand` have their own execute 
# function. Ones here are global.
...
def arg_parser():
    ...
    * ``bucket-name`` is positional argument for the ``snap-s3`` 
    ...
    sub_parsers = main_parser.add_subparsers(title="Commands", 
        description="Commands to work with Cassandra backups.")

    # Start adding sub commands
    # we add a default arg that points to the function to run for each 
    # command.
    ...
    for entry_point in pkg_resources.iter_entry_points("sub_commands"):
